...
export function funTips(opts: {
  ...
  const tips: Tip[] = []
  const add = (emoji: string, title: string, body: string) => tips.push({ emoji, title, body })
  ...
  if (snow) {
    if (today.tmax <= 0)
      add('☃️', '빙판길 주의', '종일 영하에 눈까지 와요. 쌓인 눈이 그대로 얼어붙는 날이니 미끄럼 방지 되는 신발에, 주머니 손 넣고 걷기 금지! 운전은 되도록 대중교통으로 바꾸세요.')
    else
      add('🌨️', '눈길 조심', '오늘 눈 소식이 있어요. 길이 미끄러울 수 있으니 평소보다 일찍 나서고, 접지력 좋은 신발을 신는 게 좋아요. 운전하신다면 차간거리를 넉넉하게.')
  } else if (rain >= 50)
    add('🌧️', '장우산 + 방수 신발', `오늘 비가 아주 많이 와요(예상 ${round1(rain)}mm). 장우산에 방수 신발까지 갖추고, 하천 산책로나 지하차도 근처는 피하세요. 이동 일정은 여유 있게 잡는 게 안전합니다.`)
  else if (gust >= 60 || wind >= 40)
    add('💨', '강풍, 우비 추천', `바람이 매우 강한 날이에요(순간 최대 ${Math.round(gust || wind)}km/h). 우산은 뒤집히기 십상이니 비가 온다면 우비가 낫고, 간판이나 떨어지는 물건도 조심하세요.`)

  // ── 2순위: 비 (확률·양 구간별, 호우는 위에서 처리됨)
  if (!snow && rain < 50) {
    if (rain >= 20)
      add('☔', '튼튼한 장우산', `오늘 ${round1(rain)}mm쯤 꽤 오는 비예요. 튼튼한 장우산에 젖어도 되는 신발을 추천해요. 바짓단은 오늘만 살짝 접어주세요.`)
    else if (prob >= 80)
      add('☂️', '우산 필수', `비가 거의 확실해요(확률 ${prob}%). 현관에서 우산 챙겼는지 한 번만 더 확인하세요. 양은 많지 않아도 맞고 다니기엔 충분히 젖습니다.`)
    else if (prob >= 60)
      add('☂️', '우산 챙기기', `강수확률 ${prob}%, 우산을 챙기는 쪽이 이기는 날이에요. 짐이 많다면 가벼운 접이식이라도 가방에 넣어두세요.`)
    else if (prob >= 40)
      add('🌂', '접이식 우산', `강수확률 ${prob}%로 애매한 하늘이에요. 접이식 우산 하나 가방에 넣어두면 마음이 편합니다. 안 오면 다행이고요.`)
    else if (prob >= 20 && today.tmax >= 27)
      add('🌦️', '소나기 가능성', '한낮 소나기 가능성이 살짝 있어요. 세차는 내일로 미루는 게 정신 건강에 좋을지도 몰라요.')
    else if (yesterday.precipSum >= 0.5 && prob < 20)
      add('🌤️', '빨래 찬스', '어제 내리던 비가 오늘은 그쳐요. 미뤄뒀던 빨래를 돌리기 딱 좋은 날이고, 눅눅해진 이불도 한번 털어 널어보세요.')
  }

  // ── 3순위: 어제 대비 급변
  const dMax = round1(today.tmax - yesterday.tmax)
  if (dMax <= -7)
    add('🧊', '어제보다 훨씬 추움', `어제보다 낮 기온이 ${Math.abs(dMax)}°나 뚝 떨어져요. 어제 옷차림은 완전히 잊고 한 단계 두껍게 입으세요. 감기 걸리기 딱 좋은 날씨입니다.`)
  else if (dMax <= -4)
    add('🧥', '겉옷 하나 더', `어제보다 낮이 ${Math.abs(dMax)}° 서늘해요. 어제 입던 대로 나가면 후회하니 겉옷 하나 꼭 걸치세요. 따뜻한 음료 한 잔도 좋은 선택.`)
  else if (dMax >= 7)
    add('🥵', '어제보다 훨씬 더움', `어제보다 ${dMax}°나 확 더워져요. 어제 기억은 버리고 한여름 모드로 입으세요. 차 안에 뒀던 물병도 오늘은 뜨거워집니다.`)
  else if (dMax >= 4)
    add('🌡️', '한 겹 덜 입기', `어제보다 ${dMax}° 더 더워요. 한 겹 덜 입는 게 정답이고, 야외 일정은 한낮을 피하면 한결 낫습니다.`)

  // ── 4순위: 절대 기온 옷차림 (낮 최고 기준)
  const t = today.tmax
  if (t >= 35)
    add('🥵', '위험한 더위', `낮 최고 ${round1(t)}°, 위험한 더위예요. 한낮 야외활동은 최대한 피하고 물을 수시로 마시세요. 어르신과 아이는 특히 조심해야 하는 날입니다.`)
  else if (t >= 33)
    add('🔥', '폭염, 양산 추천', `낮 최고 ${round1(t)}° 폭염이에요. 통풍 잘 되는 옷에 양산이 의외로 큰 도움이 됩니다. 실내외 온도차가 크니 냉방병도 슬쩍 조심.`)
  else if (t >= 31)
    add('💧', '물 자주 마시기', `한낮이 ${round1(t)}°까지 올라 푹푹 쪄요. 목마르기 전에 물을 미리 마시고, 뙤약볕 일정은 짧게 끊어 가세요.`)
  else if (t >= 28)
    add('☀️', '반팔 + 얇은 겉옷', `낮엔 ${round1(t)}°까지 올라 반팔이 맞아요. 다만 실내 냉방이 셀 수 있으니 얇은 겉옷 하나면 완벽합니다.`)
  else if (t >= 25)
    add('😌', '반팔 날씨', rainsToday ? `낮 최고 ${round1(t)}°, 비만 아니면 반팔이 딱 좋은 온도예요. 젖어도 금방 마르는 옷이 편합니다.` : `낮 최고 ${round1(t)}°, 반팔이나 아주 얇은 긴팔이 딱 좋은 날이에요. 활동하기 좋습니다.`)
  else if (t >= 21)
    add('🍃', '가벼운 긴팔', `낮 ${round1(t)}°로 쾌적한 날씨예요. 가벼운 긴팔 하나로 충분하고, 산책이나 야외 일정 잡기 좋은 날입니다.`)
  else if (t >= 17)
    add('🍂', '긴팔 + 겉옷', `낮에도 ${round1(t)}°라 선선해요. 긴팔에 가벼운 겉옷 조합을 추천해요. 해 지면 제법 쌀쌀해집니다.`)
  else if (t >= 12)
    add('🧥', '니트·자켓', `낮 최고가 ${round1(t)}°에 그쳐요. 니트나 자켓 정도는 입어야 하는 날씨입니다. 얇게 나가면 종일 웅크리게 돼요.`)
  else if (t >= 5)
    add('🧤', '코트·패딩', `종일 추워요(낮 최고 ${round1(t)}°). 코트나 패딩을 꺼낼 때가 됐습니다. 목만 따뜻해도 체감이 확 달라져요.`)
  else if (t >= 0)
    add('⛄', '패딩 풀장착', `낮에도 ${round1(t)}°밖에 안 돼요. 두꺼운 패딩에 목도리, 장갑까지 풀장착을 추천합니다. 따뜻한 음료 텀블러도 챙기세요.`)
  else
    add('🥶', '혹한, 핫팩', `낮 최고가 영하 ${Math.abs(round1(t))}°인 혹한이에요. 핫팩을 챙기고 피부 노출을 최소화하세요. 수도 동파도 조심할 날입니다.`)

  // ── 5순위: 일교차·중간 바람
  if (today.tmax - today.tmin >= 12 && today.tmax >= 20)
    add('🌅', '일교차 큼, 레이어드', `아침 ${round1(today.tmin)}°, 낮 ${round1(today.tmax)}°로 일교차가 커요. 입고 벗기 쉬운 레이어드로 나가면 하루 종일 편합니다.`)
  if (wind >= 25 && wind < 40 && gust < 60)
    add('🍃', '바람막이 추천', `바람이 제법 부는 날이에요(최대 ${Math.round(wind)}km/h). 얇은 옷은 바람에 뚫리니 바람막이가 든든하고, 체감온도는 숫자보다 낮게 느껴집니다.`)

  // ── 6순위: 자외선
  if (uvMax !== null) {
    if (uvMax >= 8)
      add('🧴', '선크림 필수', `자외선 지수 ${round1(uvMax)}, 매우 강해요. 선크림 필수에 한낮엔 모자나 양산까지. 야외에 오래 있다면 두세 시간마다 덧발라 주세요.`)
    else if (uvMax >= 6 && !rainsToday)
      add('🕶️', '선크림 추천', '자외선이 강한 편이에요. 잠깐 나가는 길이라도 선크림을 발라두면 피부가 고마워합니다. 선글라스 쓰기 좋은 날.')
  }
